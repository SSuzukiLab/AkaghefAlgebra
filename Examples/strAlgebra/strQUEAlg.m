classdef(InferiorClasses=?sym) StrQUEAlg<StrAlg&HopfAlg
    ...
    methods(Static)
        function [O,E,F,K,Ki,H]=getGenerator(Nvar)
            arguments
                Nvar {mustBeInteger}
            end
            O=StrQUEAlg();
            ...
            O=O.make(0,{[]});
            C=num2cell(1:Nvar);
            C(2:6,:)=reshape(arrayfun(@(i){O.make(1,{i})},1:5*Nvar),Nvar,5).';
            E=dictionary(C{[1 2],:});
            F=dictionary(C{[1 3],:});
            K=dictionary(C{[1 4],:});
            Ki=dictionary(C{[1 5],:});
    ...
    methods
        ...
        function ret = Delta(obj)
            arr=obj.DeltaStorage.get(obj.Ntype);
            I=arr(end);
...
function ret = createDelta(Ntype)
    ...
    I_=O.unit;
    ret=[arrayfun(@(n)(E(n)|K(n))+(I_|E(n)),1:N),...
        arrayfun(@(n)(F(n)|I_)+(Ki(n)|F(n)),1:N),...
        arrayfun(@(n)K(n)|K(n),1:N),...
        arrayfun(@(n)Ki(n)|Ki(n),1:N),...
        arrayfun(@(n)(H(n)|I_)+(I_|H(n)),1:N),...
        I_];
    % ret=[arrayfun(@(n)(E(n)|I_)+(K(n)|E(n)),1:N),...
...
function ret = createRel(Nvar)
    % createRel: Returns relationships based on Nvar
