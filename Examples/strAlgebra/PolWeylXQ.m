classdef(InferiorClasses=?Pol) PolWeylXQ<PolAlg
    ...
    methods(Static)
        function v=getGenerator(l)
            v=rowfun(@(x)PolWeylXQ(1,x),table(eye(3*l)));
            v=dictionary((1:3*l)',v.Var1);
        end
    end
    methods
        function obj=PolWeylXQ(varargin)
            obj@PolAlg(varargin{:})
            obj.Nvar=obj.dim/3;
