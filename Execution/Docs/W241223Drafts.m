...
Introduction
シナリオ
    非可換環の例としてリー代数の普遍包絡を挙げます(そういうモノがあるんだな程度でOK)。たとえばはリー代数であり、という3つの生成元からなる代数で

という3つの関係式から成ります。ここではリー括弧です。
さらに
という余積構造により双代数構造が入ります。
    例えば、という等式が出てきたとしましょう。これなら面倒くさいですが、定義関係式を何回か用いることで、できます。しかし、
...
今回の制作物の紹介
    ...
    そこから派生させて、のクラス(名前はそれぞれStrWeylAlg, Usl2, Uqsl2)を作りました。詳細は省きますが、それぞれの代数を関係式に従って簡約化する機能もあります。(たとえばならなど)
　任意の結合的かつ単位的な代数は生成元と関係式の表示により記述できるので、リー代数であれば、という3つの生成元と次の関係式で定義出来ます:

PBWの定理により、任意のの元はの形の項の線形結合で書けます。
...
生成元,自然数
生成元の積,数値の配列
複数項, 同じサイズの係数と指数の配列のセル配列
テンソル積因子,セル配列の横方向の配列

という対応関係で表されていて、関係式は0と等しいStrAlgオブジェクトの配列で表されます。
...
たとえばU(sl2)の場合、E:1. F:2, H:3という順番になるように簡約化していきます。
FEという項は、配列が[2 1]なので、[1 2]となるように簡約化します。
それで、FE=EF-Hという関係式を検索して、適用します。
...
のように、どうしても使いたくなる場面があるときのベストプラクティスはあまり知られていない気がします。でもハンドルクラスを使えば解消できます！
    より具体的に問題を見ます。計算結果に対して色々な表示方法があります。たとえばであれば
disp1(F*E)
disp2(F*E)
