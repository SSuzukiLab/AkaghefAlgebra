        ...
        function ret=HP(i1,i2)
            ...
            elseif i1.term*i2.term==0
                ...
                elseif length(p1)>1||length(p2)>1
                    ...
                    hp1=arrayfun(@(y)HP(X3.set_cp(X3.cf,{X3.pw{1}(1)}),y),X12(:,1),UniformOutput=false);
                    hp2=arrayfun(@(y)HP(X3.set_cp(ones(size(X3.cf)),{X3.pw{1}(2:end)}),y),X12(:,2),UniformOutput=false);
                    % hp3=[X12(:,1).cf].';
