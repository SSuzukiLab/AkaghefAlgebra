classdef (InferiorClasses={?sym})symp
    % symp 多項式クラス
    ...
    methods
        ...
        function obj=symp(i1,i2)
            ...
            obj.cf=feval(obj.ctype.class,i1);
            obj.pw=feval(obj.ptype.class,i2);
            obj=obj.C();
        end
        %% cast type
        ...
        function disp(i1)
            if isempty(i1)
                disp("Empty symp: ("+join(string(size(i1)),",")+")")
            else
                for ii=1:numel(i1)
                    disp_(i1(ii))
                end
        ...
        function ret=convertTermToSym(obj)
            ...
            try
                ...
                error("symp:disp","cannot convert coefficient to sym")
            end
        end
        % convert to sym
...
function ret=combineTerm(i1)
    ...
    % ret=i1.set_cp(S,u);
    [v,vidx]=sortrows(p);
    S=zeros(numel(vidx),1,i1.ctype.class);
    c=c(vidx);
